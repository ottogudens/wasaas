...
jest.mock('../src/utils', () => ({
    sherpaCleanNumber: jest.fn().mockImplementation(() => phoneNumber),
    sherpaIsValidNumber: jest.fn((number: string) => {
        if (!number || number.trim() === '') return false
        return !number.includes('@g.us')
    }),
    sherpaGenerateImage: jest.fn(),
...
describe('#SherpaProvider', () => {
    ...
    beforeEach(() => {
        ...
        provider = new SherpaProvider(args)
        mockReq = {}
        mockRes = {
            writeHead: jest.fn(),
            end: jest.fn(),
    ...
    test('should initialize SherpaProvider correctly with default arguments', () => {
        // Arrange
        const defaultArgs = {
            name: 'bot',
            gifPlayback: false,
        ...
        // Act
        const sherpaProvider = new SherpaProvider({})

        // Assert
        expect(sherpaProvider.globalVendorArgs).toEqual(defaultArgs)
    })
    ...
    describe('busEvents - messages.update', () => {
        test('Survey update received', async () => {
            ...
            const originalGetAggregateVotes = require('whaileys').getAggregateVotesInPollMessage
            require('whaileys').getAggregateVotesInPollMessage = jest.fn().mockReturnValue({})
            ...
            try {
                ...
                require('whaileys').getAggregateVotesInPollMessage = originalGetAggregateVotes
            }
        })
    })
